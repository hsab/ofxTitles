...
void ofxTitles::bindTime(int duration) {
   // add a blank subtitle, with no duration to the end of the movie to make looping work
   add("", INT_MAX, duration * 1000, duration * 1000);
}
...
void ofxTitles::draw(void) {
   ofColor color(255, 196, 0);
   draw(0, 0, color);
}

void ofxTitles::drawCenter(int size, int x, int y, ofColor& color) {
   if (playback_state != TITLE_DISPLAYING)
      return;

   myText.setText((*play_head)->text);
   myText.wrapTextX(size);
   ofSetColor(color);
   myText.drawCenter(x, y);
}

void ofxTitles::draw(float x, float y, ofColor& color) {
...
void ofxTitles::draw(float x, float y, float w, float h, ofColor& color, float percent) {
   // can't draw if there's nuthin' ta draw
   if (playback_state != TITLE_DISPLAYING)
      return;
